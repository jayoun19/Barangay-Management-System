<x-app-layout>
    <x-slot name="header">Edit Asset</x-slot>
        ...
        <form action="{{ route('assets.update', $asset) }}" method="POST" class="space-y-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $asset->name) }}" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm" required>
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-slate-700">Category</label>
                    <input type="text" name="category" value="{{ old('category', $asset->category) }}" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="quantity" value="{{ old('quantity', $asset->quantity) }}" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm" min="0" required>
                </div>
                <div class="lg:col-span-2">
                    <label class="mb-2 block text-sm font-medium text-slate-700">Description</label>
                    <textarea name="description" rows="4" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">{{ old('description', $asset->description) }}</textarea>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('assets.index') }}" class="rounded-2xl border border-slate-200 px-5 py-3 text-sm text-slate-700">Back</a>
                <button type="submit" class="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white">Update Asset</button>
            </div>
        </form>
    </div>
</x-app-layout>
